...
```
요청이 들어옴
   └─▶ pm:task-draft      맥락표 — 누가 무엇을 요청했고, 아직 아무도 안 정한 건 무엇인지
        └─▶ pm:prd        기획 — 만들고 나면 무엇이 참이어야 하는지
             └─▶ fig      prep · 그리기 · arrows · lint · diff, 그리고 핸드오프
                  └─▶ pm:task-publish    개발이 집어가는 티켓
                       └─▶ 배포됨
                            ├─▶ fig:qa     같은 기획서를 근거로 판정
                            └─▶ fig:sync   정본 페이지를 배포된 것까지 최신화
```
