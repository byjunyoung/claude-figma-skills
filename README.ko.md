...
**`adapter.py`가 4로 끝납니다 — 어댑터가 반대쪽 역할입니다.**
그 타입의 파일은 있지만 `roles:` 줄이 맡는 쪽을 말하고 스킬은 반대쪽을 물었습니다 — 내장 GitHub 파일은 미러, Notion·markdown 파일은 레코드입니다. 반대쪽에 놓은 설정은 어댑터 반쪽으로 도는 대신 여기서 멈춥니다. `/pm:setup` 3b가 빠진 쪽을 초안합니다.
