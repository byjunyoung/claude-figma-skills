---
name: figma-prep
description: Figma 디자인 페이지를 본 작업 전에 준비시킨다 — 팀 관리 가이드에 맞게 프레임 네이밍(`[화면명]-[상태]`) 통일, 기능 단위 Section 생성·배치, 화면 내용을 읽어 누락 케이스(상태 변형·인터랙션 결과 화면)를 검출하고 placeholder 프레임으로 채움. 로컬 폰트 파일의 reparent 제약을 우회하는 흡수 워크플로 포함. 순번 재부여·핸드오프 URL 출력까지 맡고, 검증·위반 리포트는 하지 않는다(현황 파악도 완료 확인도 /figma-lint). Triggers - "/figma-prep", "프렙 해줘", "가이드대로 정리해줘", "프레임 정리해줘", "섹션으로 묶어줘", "빠진 케이스 채워줘", "placeholder 만들어줘".
...
# figma-prep — 페이지 준비 (정리·섹션화·누락 케이스 placeholder)

요리의 *mise en place*처럼, 본 디자인·화살표 작업 전에 프레임이라는 재료를 전부 썰어 제자리에 갖춰두는 단계다.

디자인 파일의 한 페이지를 받아 (1) 프레임 이름을 규칙에 맞게 통일하고, (2) 기능 단위 Section으로 묶어 배치하고, (3) 각 화면의 실제 내용을 읽어 빠진 케이스를 placeholder 프레임으로 채운다. 정리가 끝난 페이지는 `/figma-arrows`로 흐름 화살표를 잇는 것까지가 한 세트다.
...
- 흐름 화살표 생성·sync → `/figma-arrows`
- 규칙 위반 검증만(쓰기 0) → `/figma-lint`
- 작업분이 정본에 반영됐는지 감사·반영 → `/figma-sync`
- 화면 자체의 디자인 생성 → `figma:figma-generate-design`
- 파일 전체 구조 파악만 → `/figma-read`

## Inputs

- `figma_url` (필수): 정리할 페이지 URL. 페이지 prefix를 보고 적용 엄격도를 정한다
- `mode` (선택): 정리 실행. 생략하면 `/figma-lint` 로 현황을 먼저 파악해 정리안을 제안한다

## 규칙 원천 — 설정 파일 (figma-lint·figma-sync와 동일 출처)
...
- 설정이 없는 낯선 파일이면 `/figma-setup` 으로 관례를 역추출해 초안을 만든 뒤 시작한다
...
3. **이미 흩어진 placeholder가 있으면 제거 후 전환** — 화면별로 만들어둔 공통-성격 placeholder 프레임은 삭제하고, 거기 걸린 `[state]`·`-->` 화살표 중 삭제 대상을 가리키던 것도 함께 정리(끊긴 흐름은 `/figma-arrows`로 재연결). **삭제 전 그 화면에 별도 Default 원본이 있는지 확인**(유일본 삭제 방지) → 미리보기 → go.
...
정리 전에 무엇이 어긋나 있는지 보려면 `/figma-lint` 를 호출한다. **검사 항목·기준·감사 코드는 전부 거기 하나에 있다** — 검출과 수정을 분리하는 Design Lint 방식이고, 이 스킬은 고치는 쪽만 맡는다.
...
**이 단계의 필수 마지막 동작 — `/figma-lint` 호출(Skill 도구로).** 쓰기(생성·복제·이동·배치)가 끝나면 **항상** `/figma-lint`를 호출해 `STRUCT PASS`·`FLOW PASS`를 받는다(특히 clone/move가 끼면 무조건). 위반이 나오면 고치고 재호출하며, **PASS 없이는 완료로 보고하지 않는다.**

자가확인용 인라인 감사를 여기 두지 않는다. 같은 검사를 두 벌 들고 있으면 한쪽만 고쳐져 갈라진다 — 판정은 언제나 `/figma-lint` 하나가 한다.

- **격리 스크린샷만으로 PASS 판정 금지.** `node.screenshot()`·단일 노드 캡처는 프레임을 단독 렌더해서 **부모·캔버스 위치 오류를 못 잡는다** — 프레임만 보면 멀쩡해 보인다. 1차는 `/figma-lint` 의 수치 감사, 2차는 **섹션 노드 전체 스크린샷**(프레임 격리 X)이다. clone/move 가 끼면 거의 항상 여기서 사고가 난다
- **섹션별 핸드오프 URL 목록 출력** — `https://figma.com/design/{fileKey}/?node-id={섹션ID의 :를 -로}` 형식. 가이드의 "개발 전달은 Section URL 공유(Frame 개별 X)" 체크리스트에 대응
- 출력 마지막에: 추측 라벨 목록(사용자 확인 필요), TBD 정책 목록, 그리고 `/figma-arrows`로 흐름 연결 제안

## 구현 — 정리 헬퍼 프리앰블

쓰기 스크립트는 `_figma-common/scripts/prep-ops.js` 를 프리앰블로 쓴다. 설정 한 줄 + 이 파일 전문 + 실제 호출을 이어 붙여 `use_figma` 에 넣는다.
...
- 정리 직후 프레임 좌표가 바뀌므로, 기존 흐름 화살표가 있던 페이지면 `/figma-arrows` sync 를 같이 제안한다
