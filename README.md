...
Figma 파일 **한 곳**에서 만들어졌고 거기서 검증됐습니다. 다른 팀의 관례·다른 언어의 상태명에서
무엇이 깨질지는 아직 모릅니다 — `/figma-setup` 이 관례를 관측해 채우도록 만든 이유가 그것이고,
`null` 로 남기는 설계도 같은 이유입니다(모르는 것을 아는 척하지 않기 위해).

낯선 파일에서 오탐이 나오면 이슈로 알려주세요. 그게 이 도구가 좋아지는 유일한 경로입니다.
