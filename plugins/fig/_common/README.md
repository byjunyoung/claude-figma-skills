# _common — fig 플러그인 공용 층

`fig` 플러그인의 스킬 열셋이 공유하는 **규칙 원천**과 **실행 코드**가 여기 있다.
...
conventions.example.yaml   스키마 + 내장 기본값. 주석이 곧 가이드다
                           (정합성 검사기는 저장소 tools/verify.py 로 옮겼다)
scripts/
  ...
  deck-base.js             Figma Slides 빌드 공통 헬퍼 (/fig:deck 용, 값 없음)
  lib/
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/_common/verify.py
```
...
2  python3 _common/verify.py          위반 0 확인
