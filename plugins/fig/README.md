...
The skills that write — `prep`, `arrows`, `diff`, `sync`, `tokens`, `deck` — check the seat with `whoami` before their first write, and stop on a View seat instead of failing halfway.
