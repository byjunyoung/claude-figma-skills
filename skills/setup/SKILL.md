...
각 페이지마다 `${CLAUDE_PLUGIN_ROOT}/_figma-common/scripts/probe-page.js` 앞에 `const PAGE_ID = "<id>";` 를 붙여 `use_figma` 로 실행한다. **페이지별로 호출을 나눠 한 메시지에서 병렬로** 띄운다(스크립트당 `setCurrentPageAsync` 1회).
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/_figma-common/scripts/lib/draft-conventions.py probe1.json probe2.json ... > draft.yaml
```
