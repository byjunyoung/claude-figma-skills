...
# fig:proto — Figma 디자인 → 동작 프로토타입 (코드 재현)
...
- 프로덕션 코드로 이식(프론트 레포 반영) → `/fig:code`
- 프레임 목록·구조만 보기 → `/fig:read`
- 그려진 화면의 구조·흐름 검증(lint) → `/fig:lint`
...
경로·퍼블리시 대상은 `figma-conventions.yaml` 의 `tools` 절이 정한다 — `python3 ${CLAUDE_PLUGIN_ROOT}/_figma-common/scripts/lib/resolve-config.py` 로 읽는다.
...
- 관련 스킬: `/fig:read`(구조만), `/fig:code`(프론트 레포 반영), `/fig:lint`(그린 화면 검증).
