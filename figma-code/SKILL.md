---
name: figma-code
description: Figma 시안을 프론트엔드 레포의 실제 코드에 반영한다 — 시안이 바뀐 자리를 기존 코드에 최소 수정으로 옮기거나, 시안에만 있고 코드에 없는 화면·상태 변형을 새로 구현한다. 레포의 기존 명명·토큰·상태 관리 관례를 먼저 읽어 덮어쓰지 않고, 수치 대조와 스크린샷 대조를 모두 거친 뒤 브랜치·PR로 낸다. Triggers - "/figma-code", "시안 코드에 반영해줘", "피그마 바뀐 거 앱에 적용", "이 화면 프론트에 구현해줘", "시안대로 코드 고쳐줘", "디자인 변경분 코드에".
...
# figma-code — Figma 시안 → 프론트 레포 코드 반영
...
- 무빌드 단일 HTML 동작 프로토타입 → `/figma-proto`
- 프레임 목록·구조만 파악 → `/figma-read`
- 시안 자체의 구조·흐름·네이밍 검증 → `/figma-lint`
- AS-IS/TO-BE 변경점을 시안에 표기하고 기획 문서에 정리 → `/figma-diff`
- 코드가 아니라 Figma 정본에 반영 → `/figma-sync`
...
- 시안 자체가 어긋나 보이면(누락 상태, 겹친 요소) 반영을 멈추고 `/figma-lint`로 넘긴다.
