...
# figma-tokens — 디자인시스템 토큰 바인딩 검수·정비
...
| 작업용 스캐폴딩 | 설정 `naming` 의 스캐폴딩 이름 규약(`arrow_delimiter`·`label_prefix`·`state_chain_prefix`)에 걸리는 노드와 `section_style`·`placeholder_style` 로 칠해진 것 — `/figma-arrows`·`/figma-prep` 산출물이라 토큰화 대상이 아니다 |
